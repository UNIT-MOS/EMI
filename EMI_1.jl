...
# ╔═╡ 62f6504c-b598-11f1-9274-41db62cb1941
using Handcalcs, PlutoUI, LaTeXStrings, PlutoPlotly;

# ╔═╡ f5daf1a9-4dc9-4016-83e1-09ebed17833b
PlutoUI.TableOfContents()

# ╔═╡ 0c690470-0749-4f4d-a724-552b5ab81b2e
md"# Definitions"

# ╔═╡ a79128db-f73e-447e-8512-a063a7843138
md"""## Electrostatic discharge
Parasitic parameters on PCB cause high voltage, current.

``C_{par}^{'} = 5\frac{pF}{cm}``

``L_{par}^{'} = 10\frac{nH}{cm}``

``U=L_{par}\frac{di}{dt}=10nH\cdot \frac{10A}{1ns}=100V``

``I=C_{par}\frac{du}{dt}=5pF\cdot \frac{3kV}{1ns}=15A``
"""

# ╔═╡ ee771247-770e-41d5-920f-0edcfd2a7878
md""" ## Switching inductive loads
When opening a switch, relay connected to an inductive load, voltage will be induced through its contacts. Voltage could be several kV, causing a light arc.
"""

# ╔═╡ ae07d2e9-0e16-42c1-a4ee-1514a20388f3
md"# Coupling mechanism"

# ╔═╡ aed928a8-58c1-45a2-8845-b71cb540b4ac
md""" ## Common mode
Common mode
- U, I flows one way
  - ground is reference potential for all voltages
- ``u_{cm} = \frac{u_{1}-u_{2}}{2}``
- ``i_{cm}=\frac{u_{cm}}{Z_{cm}}``
- prone to EMI because of synchronous clock
"""
...
- capacitive coupling
  - caused by ``Z_{2} \rightarrow \infty``
- inductive coupling
  - caused by ``Z_{2} \rightarrow 0``
"""

# ╔═╡ fb8de777-78c0-4787-9a42-b03e8765c616
md""" ### Coupling on PCB
- coupling impedance of ground: ``Z_{O} = R_{O}+j\omega L_{O}``
- minimizing coupling through small current loops, high distance between tracks
"""
